...
export function AreaCard({ area }: { area: Area }) {
  ...
  const allActive = activeCount === area.equipment.length
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-arcade-blue-gray/20 bg-arcade-dark">
      <button
        type="button"
        onClick={handleExpand}
        className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-arcade-blue-gray/10"
      >
        ...
        <span
          className={`text-arcade-blue-gray transition-transform duration-200 ${
      ...
      {isExpanded && (
        <div className="border-t border-arcade-blue-gray/10 px-2 py-2">
          <div className="mb-2 flex justify-end px-3">
            <button
              type="button"
              onClick={handleToggleAll}
              className="text-xs font-semibold text-arcade-neon-magenta hover:text-arcade-neon-yellow transition-colors"
            >
